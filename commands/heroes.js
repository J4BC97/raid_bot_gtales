...
module.exports = {
  ...
  async execute(interaction) {
    ...
    try {
      ...
      if (!heroDetails) {
        return interaction.reply({ content: translations[lang].heroNotFound, flags: MessageFlags.Ephemeral }); // Cambiar aquí
      }
      ...
      if (!heroDetails.available) {
        ...
        return interaction.reply({ embeds: [embed], content: translations[lang].heroInfoIncomplete, flags: MessageFlags.Ephemeral }); // Cambiar aquí
      }
      ...
      await interaction.reply({ embeds: [heroEmbed], flags: MessageFlags.Ephemeral }); // Cambiar aquí
    } catch (error) {
      console.error('Error fetching hero details:', error);
      return interaction.reply({ content: translations[lang].heroDetailsError, flags: MessageFlags.Ephemeral }); // Cambiar aquí
    }
  },
